...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, silo_proc_num=1):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    for client_idx in range(client_number):
        ...
        proc_data_idxs = np.array_split(dataidxs, silo_proc_num)
        print()
        for proc_rank in range(silo_proc_num):
            # training batch size = 64; algorithms batch size = 32
            train_data_local, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size,
                                                    proc_data_idxs[proc_rank])
