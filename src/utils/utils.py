...
def get_benign_updates(mali_clients, server):
    ...
    for name in mali_clients[0].W:
        ...
        mal_user_grad_std2[name] = torch.sqrt(
            (
                mal_user_grad_pow[name] / len(mali_clients)
                - torch.pow(mal_user_grad_mean2[name], 2)
            )
        )
...
def plot_2d(data, y, real, save_name):
    ...
    for i, point in enumerate(data.data):
        ...
        if y[i] == 0:
            ...
            ax.scatter(
                *point, zorder=i, color="#dbe9ff", alpha=0.6, edgecolors=colors[5]
            )
        else:
            ax.scatter(
                *point, zorder=i, color="#ffdbdb", alpha=0.6, edgecolors=colors[1]
            )

    handles = [
        plt.Line2D([0], [0], color="w", lw=4, label="Ground Truth Benign"),
        ...
        for x, y in loader:
            ...
            if lambda_fedprox > 0.0:
                loss += lambda_fedprox * torch.sum(
                    (
                        flatten(W0).cuda()
                        - flatten(dict(model.named_parameters())).cuda()
                    )
                    ** 2
                )
...
def train_op(
    model, loader, optimizer, epochs, lambda_fedprox=0.0, print_train_loss=False
):
    model.train()
...
def train_op_private(
    ...
    for ep in range(epochs):
        ...
        for it, (x, y) in enumerate(loader):
            ...
            # add Gaussian noise
            for name, param in model.named_parameters():
                clipped_grads[name] += gaussian_noise(
                    clipped_grads[name].shape, clip_bound, privacy_sigma, device="cuda"
                )
            for name, param in model.named_parameters():
...
def train_op_datadistill(
    ...
    distilled_loader = torch.utils.data.DataLoader(
        distilled_dataset, batch_size=32, shuffle=True, num_workers=4
    )
...
def kd_loss(output, y):
    soft_label = torch.softmax(y, dim=1)
    # soft_label = y
    # logsoftmax = torch.nn.LogSoftmax()
    return torch.mean(-soft_label * torch.log_softmax(output))


def train_op_datadistill_soft(
    model,
    loader,
    optimizer,
    epochs,
...
def train_op_datadistill_later(
    model,
    loader,
    optimizer,
    epochs,
    ...
    distilled_dataset = TensorDataset(images_train, labels_train)
    distilled_loader = torch.utils.data.DataLoader(
        distilled_dataset, batch_size=256, shuffle=True, num_workers=4
    )
...
def eval_op_ensemble_nlp(models, test_loader, val_loader):
    ...
    with torch.no_grad():
        for label, text, offsets in test_loader:
            ...
            y_ = torch.mean(
                torch.stack([model(text, offsets) for model in models], dim=0), dim=0
            )
            _, predicted = torch.max(y_.detach(), 1)
...
def reduce_trimmed_mean(target, sources, mali_ratio):
    ...
    for name in target:
        ...
        smallest_value, _ = torch.topk(
            stacked_weights, k=trimmed_mean_beta, dim=0, largest=False
        )
        target[name].data = (
            (
...
def reduce_weighted(target, sources, weights):
    for name in target:
        # import pdb; pdb.set_trace()
        target[name].data = torch.sum(
            weights
            * torch.stack([source[name].detach() for source in sources], dim=-1),
            dim=-1,
        ).clone()
        # import pdb; pdb.set_trace()
