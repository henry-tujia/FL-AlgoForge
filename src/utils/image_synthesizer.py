...
from src.utils.utils import kd_loss, DiffAugment
# import warnings
...
def reduce_params(sources, weights):
    ...
    for i in range(len(sources[0])):
        target = torch.sum(
            weights * torch.stack([source[i].cuda() for source in sources], dim=-1),
            dim=-1,
        )
...
def epoch(mode, dataloader, net, optimizer, criterion, aug=True, args=None):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if aug:
            img = DiffAugment(
                img, args.method.dsa_strategy, param=args.method.dsa_param
            )
        n_b = lab.shape[0]
        output = net(img)
...
def evaluate_synset(it_eval, net, lr_net, images_train, labels_train, testloader, args):
    ...
    trainloader = torch.utils.data.DataLoader(
        dst_train, batch_size=256, shuffle=True, num_workers=0
    )
    start = time.time()
    acc_train_list = []
    loss_train_list = []
    for ep in tqdm.tqdm(range(Epoch + 1)):
        loss_train, acc_train = epoch(
            "train", trainloader, net, optimizer, kd_loss, aug=True, args=args
        )
...
class Synthesizer:
    def __init__(self, network, test_loader, args):
        ...
        self.dataset = args.datasets.dataset
        self.testloader = test_loader
        self.batch_syn = args.method.batch_syn
        self.save_path = args.paths.output_dir
        self.iteration = args.method.Iteration
        hard_label = [
            np.ones(args.method.ipc, dtype=np.long) * i
            for i in range(args.datasets.num_classes)
        ]
        label_syn = torch.nn.functional.one_hot(
            torch.tensor(hard_label).reshape(-1), num_classes=args.datasets.num_classes
        ).float()
        label_syn = label_syn * args.method.label_init
        label_syn = label_syn.detach().to(self.device).requires_grad_(True)
        single_img_shape = test_loader.dataset.data.shape[1:]
        image_syn = torch.randn(
            size=single_img_shape,
            dtype=torch.float,
        )
        syn_lr = torch.tensor(args.method.lr_teacher).to(self.device)
        image_syn = image_syn.detach().to(self.device).requires_grad_(True)
        syn_lr = syn_lr.detach().to(self.device).requires_grad_(True)
        if args.method.img_optim == "sgd":
            optimizer_img = torch.optim.SGD(
                [image_syn], lr=float(args.method.lr_img), momentum=0.5
            )
            optimizer_label = torch.optim.SGD(
                [label_syn], lr=float(args.method.lr_label), momentum=0.5
            )
        else:
            optimizer_img = torch.optim.Adam([image_syn], lr=float(args.method.lr_img))
            optimizer_label = torch.optim.Adam(
                [label_syn], lr=float(args.method.lr_label)
            )
        if args.method.lr_optim == "sgd":
            optimizer_lr = torch.optim.SGD(
                [syn_lr], lr=float(args.method.lr_lr), momentum=0.5
            )
        else:
            optimizer_lr = torch.optim.Adam([syn_lr], lr=float(args.method.lr_lr))
        self.test_loader = test_loader
        self.label_syn, self.image_syn, self.syn_lr = label_syn, image_syn, syn_lr
        self.optimizer_img, self.optimizer_label, self.optimizer_lr = (
            optimizer_img,
            optimizer_label,
            optimizer_lr,
        )
        ...
        (
            self.weight_averaging,
            self.least_ave_num,
            self.max_ave_num,
            self.random_weights,
        ) = (
            args.method.weight_averaging,
            args.method.least_ave_num,
            args.method.max_ave_num,
            args.method.random_weights,
        )
        self.distributed = torch.cuda.device_count() > 1
        (
            self.syn_steps,
            self.min_start_epoch,
            self.max_start_epoch,
            self.expert_epochs,
        ) = (
            args.method.syn_steps,
            args.method.min_start_epoch,
            args.method.max_start_epoch,
            args.method.expert_epochs,
        )

    def synthesize(self, trajectories_list, args):
        for it in range(0, self.iteration):
            trajectories = trajectories_list[
                random.randint(0, len(trajectories_list) - 1)
            ]
            ...
            curr_max_start_epoch = min(
                [self.max_start_epoch, len(trajectories) - 1 - self.expert_epochs]
            )
            if curr_max_start_epoch == 0:
                start_epoch = 0
            else:
                start_epoch = np.random.randint(
                    self.min_start_epoch, curr_max_start_epoch + 1
                )
            ...
            if not self.weight_averaging:
                ...
                max_ave_num = (
                    self.max_ave_num + 1
                    if self.max_ave_num < self.expert_epochs
                    else self.expert_epochs + 1
                )
                averaging_num = random.choice(
                    list(range(self.least_ave_num, max_ave_num))
                )
                candidate_params = random.choices(
                    trajectories[
                        start_epoch + 1 : start_epoch + self.expert_epochs + 1
                    ],
                    k=averaging_num,
                )
            ...
            for step in range(self.syn_steps):
                ...
                if args.method.dsa:
                    x = DiffAugment(
                        x, args.method.dsa_strategy, param=args.method.dsa_param
                    )
                if self.distributed:
                    forward_params = (
                        student_params[-1]
    ...
    def evaluate(self, args=None):
        ...
        for it_eval in range(3):
            ...
            image_syn_eval, label_syn_eval = (
                copy.deepcopy(image_save.detach()),
                copy.deepcopy(eval_labs.detach()),
        ...
        #     image_syn_vis[:, ch] = (
        #         image_syn_vis[:, ch] * std_dataset.get(self.dataset)[ch]
        #         + mean_dataset.get(self.dataset)[ch]
        #     )
        # image_syn_vis[image_syn_vis < 0] = 0.0
        # image_syn_vis[image_syn_vis > 1] = 1.0
        # grid = torchvision.utils.make_grid(
        #     image_syn_vis, nrow=10, normalize=True, scale_each=True
        # )
        # wandb.log(
        #     {
        #         "Synthetic vis_Images": wandb.Image(
        #             torch.nan_to_num(grid.detach().cpu())
        #         )
        #     },
        #     step=c_round,
        # )
        save_data_path: pathlib.Path = args.paths.output_dir / "syn_data"
        if not save_data_path.exists():
            save_data_path.mkdir(parents=True)
        torch.save(self.image_syn.detach().cpu(), save_data_path / "images_best.pt")
        torch.save(self.label_syn.detach().cpu(), save_data_path / "labels_best.pt")

        return acc_test_mean, acc_test_std
