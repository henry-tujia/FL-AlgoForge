    ...
    def operations(self, client_info):
        ...
        if self.round >= self.tau:
            for k in range(self.tau - 1, -1, -1):
                gcs = [
                    value[1]
                    for key, value in self.local_gradient_round.items()
                    if key == self.round - k and gt @ value[1].to(self.device) < 0
                ]
                if gcs:
        ...
        return [self.model.cpu().state_dict() for x in range(self.args.federated_settings.thread_number)]
