...
def conv3x3(in_planes, out_planes, stride=1, groups=1,kernel_size=3, dilation=1, width_max=1.0):
    """3x3 convolution with padding"""
    return USConv2d(in_planes, out_planes, kernel_size, stride=stride,
                     padding=0, groups=groups, bias=False, dilation=dilation, width_max=width_max)
...
class ImageNet(nn.Module):
    ...
    def extract_feature(self, x):
        ...
        if self.KD == True:
            ...
            print("extract: x3 {} x4 {}".format(x3.shape,x4.shape))
            return [x3, x4], x

    def reuse_feature(self, x):
        temp = make_divisible(x.shape[1]*self.width_mult)
        x3 = x[:, :temp]

        print("resue: x3\t",x3.shape)
        print("temp:",temp)
        x4 = self.layer4(x3)
        print("resue: x4\t",x4.shape)
        return [x3, x4] 
