...
class Client(Base_Client):
    def __init__(self, client_dict, args):
        ...
        self.prev_model = self.model_type(
            **client_dict["model_paras"]).to(self.device)
        self.global_model = self.model_type(
            **client_dict["model_paras"]).to(self.device)
...
class Server(Base_Server):
    def __init__(self, server_dict, args):
        ...
        self.prev_models = {x: self.model.cpu().state_dict()
                            for x in range(self.args.client_number)}
        # self.criterion = torch.nn.CrossEntropyLoss().to(self.device)
    ...
    def run(self, received_info):
        ...
        server_outputs = [{'global': g, 'prev': self.prev_models}
